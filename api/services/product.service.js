...
const userAgents = ['Mozilla/5.0 (Linux; Android 13; SM-S901B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36'
...
const getProductDetails = async (url) => {
    ...
    const productPrice = $('span.a-price-whole').first().text().trim().replaceAll(',', '') ||
        $('div._30jeq3._16Jk6d').first().text().trim().replace('₹', '').replaceAll(',', '');
    const productImage = $('img#landingImage').attr('src') ||
        $('img._396cs4._2amPTt._3qGmMb').attr('src') ||
        $('img._2r_T1I._396QI4').attr('src');

    if (!productTitle || !productPrice) throw new Error('Unable to fetch product details');
...
// (async () => {
//     let temp = await getProductDetails('https://www.amazon.in/Destinio-Universal-International-Worldwide-Singapore/dp/B0BQ7G5DJX/ref=sr_1_20');
//     console.log(temp);
// })();
